  ...
  export async function createJob(req, res) {
    try {
      ...
      const { position, description, yearsOfExperienceRequired = 0, skillsRequired = [], department, location, } = req.body ?? {};

      if (!position || !description) {
        return res.status(400).json({ message: 'position and description are required' });
      }

      const skills = Array.isArray(skillsRequired) ? skillsRequired.map(s => String(s).trim()).filter(Boolean)
        : (typeof skillsRequired === 'string'
          ? skillsRequired.split(',').map(s => s.trim()).filter(Boolean)
          : []);

      const payload = { position: String(position).trim(),
        description: String(description).trim(),
        yearsOfExperienceRequired: Number.isFinite(Number(yearsOfExperienceRequired)) ? Number(yearsOfExperienceRequired) : 0,
