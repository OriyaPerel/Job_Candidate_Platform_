...
async function main() {
  ...
  const col = client.db(dbName).collection("jobs");
  ...
  console.log(`🔎 jobs missing embeddings: ${totalMissing}`);
  ...
  while (await cursor.hasNext()) {
    ...
    // טקסט עמוק מכל השדות; ואם ריק—fallback מינימלי
    let text = autoTextDeep(doc);
    if (!text || !text.trim()) {
      text = `job:${doc?._id} title:${doc?.title ?? ""} company:${doc?.company ?? ""} location:${doc?.location ?? ""}`;
    }

    if (shown < 3) {
      console.log("🧩 example job text for", String(doc._id), "=>", text.slice(0, 200));
      shown++;
    }
