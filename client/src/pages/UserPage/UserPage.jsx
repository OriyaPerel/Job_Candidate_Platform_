...
export default function UserPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError("");
        const data = await getUserByEmail(email, { signal: ac.signal });
        setUser(data || null);
      } catch (err) {
        ...
        if (status === 404) {
          ...
          setError("לא הצלחתי לטעון את המשתמש");
        }
      } finally {
        setLoading(false);
      }
  ...
  if (!user)   return <div className={styles.form}><div className={styles.title}>לא נמצא משתמש</div></div>;
  ...
  <div className={styles.form}>
    ...
    <section className={styles.grid2}>
      ...
      <div className={styles.field}>
        <label>Email</label>
        <div className={`${styles.value} ${styles.mono} ${styles.break}`}>{user.email}</div>
      </div>
    ...
    {/* ======================== */}
     {/* ===== Jobs user.name looking for — רשימה בתוך ריבוע ===== */}
    <div className={styles.field}>
  <label>Jobs {user.fullName} looking for</label>
  <div className={styles.box}>
    ...
    <div className={styles.field}>
      <label>About</label>
      <div className={`${styles.value} ${styles.multiline}`}>{user.about || "-"}</div>
    </div>
